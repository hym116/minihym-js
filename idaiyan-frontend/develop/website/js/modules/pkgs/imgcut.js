...
$.ImgCut = function(container, drag, url, options) {
    ...
    Drag.prototype = {
        ...
        Start: function(oEvent) {
            ...
            window.mouseMoveEvent = this._fM;
            $(document).mousemove(this._fM);
            window.mouseUpEvent = this._fS;
        ...
        Move: function(oEvent) {
            ...
            if (this.Limit) {
                var iRight = iLeft + this._obj[0].offsetWidth - this.mxRight,
                    iBottom = iTop + this._obj[0].offsetHeight - this.mxBottom;
    ...
    Resize.prototype = {
        ...
        Start: function(oEvent, o) {
            ...
            window.mouseMoveEventFR = this._fR;
            $(document).mousemove(this._fR);
            window.mouseUpEventFS = this._fS;
            $(document).mouseup(this._fS);
            if (isIE) {
        ...
        Resize: function(e) {
            ...
            this._obj.css({
                "width": this.style_width - this._xBorder + "px",
                "height": this.style_height - this._yBorder + "px",
                "top": this.style_top + "px",
                "left": this.style_left + "px"
            });
            this.onResize();
        },
        ...
        repairLeft: function(iWidth) {
            if (iWidth < this.MinWidth) {
                iWidth = this.MinWidth;
            } else if (this.Limit && iWidth > this._mxLeft) {
                iWidth = this._mxLeft;
            }
            this.style_width = iWidth;
        ...
        repairUp: function(iHeight) {
            if (iHeight < this.MinHeight) {
                iHeight = this.MinHeight;
            } else if (this.Limit && iHeight > this._mxUp) {
                iHeight = this._mxUp;
            }
            this.style_height = iHeight;
    ...
    ImgCropper.prototype = {
        ...
        GetResize: function() {
            ...
            if (op.RightDown || op.LeftDown || op.RightUp || op.LeftUp || op.Right || op.Left || op.Up || op.Down) {
                ...
                var oThis = this,
                    _resize = new Resize(this.Drag, resOp);
                if (op.RightDown) {
        ...
        PreView: function() {
            ...
            for (var i = 0; i < this.View.length; i++) {
                ...
                if (isIE || !this._view[i].attr("angle") || this._view[i].attr("angle") % 180 == 0) {
                    ...
                    styleView.width = ph + "px";
                    styleView.height = pw + "px";
                }
                styleView.top = -pt + "px ";
    ...
    this.rotate = function(angle, whence) {
        ...
        if (this.angle >= 0) {
            rotation = Math.PI * this.angle / 180;
        } else {
            rotation = Math.PI * (360 + this.angle) / 180;
        }
