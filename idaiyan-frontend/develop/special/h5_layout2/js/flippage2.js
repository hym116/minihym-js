                    ...
                    if (moveX > 0) {
                        ...
                        if (moveUp || touchStart) {
                            ...
                            if (next) {
                                if (next.classList.contains("page")) {
                                    ...
                                    next.style.webkitTransform = "translateX(-100%)";
                                    $(next).trigger("active");
                                    current.style.webkitTransformOrigin = "right center";
                                } else {
                                    current.style.webkitTransform = "translateX(0px)";
                                    next = null;
                                }
                            }
                        } else {
                            current.style.webkitTransform = "translateX(" + moveX + "px)";
                            next.style.webkitTransform = "translateX(-" + (window.innerHeight - moveX) + "px)";
                        }
                    } else if (0 > moveX) {
                        ...
                        if (!moveUp || touchStart) {
                            ...
                            if (next && next.classList.contains("page")) {
                                ...
                                next.classList.add("z-move");
                                next.style.webkitTransition = "none";
                                next.style.webkitTransform = "translateX(" + window.innerHeight + "px)";
                                $(next).trigger("active");
                                current.style.webkitTransformOrigin = "left center";
                            } else {
                                current.style.webkitTransform = "translateX(0px)";
                                next = null;
                            }
        ...
        that._$app.on("mouseup touchend", function() {
            if (moving) {
                ...
                if (next) {
                    ...
                    if (Math.abs(moveY) < Math.abs(moveX) && Math.abs(moveX) > 100) {
                        if (moveUp) {
                            current.style.webkitTransform = "translateX(-" + window.innerHeight + "px)";
                            next.style.webkitTransform = "translateX(0px)";
                        } else {
                            current.style.webkitTransform = "translateX(" + window.innerHeight + "px)";
                            next.style.webkitTransform = "translateX(0px)";
                        }
        ...
        window._app_showPage = function(index,notrans) {
            ...
            if (this._isFirstShowPage && !(next && next.length)) {
                ...
                if (index > currentIndex) {
                    ...
                    next[0].style.webkitTransform = "translateX(" + window.innerHeight + "px)";
                    current[0].style.webkitTransform = "translateX(0px)";

                    setTimeout(function() {
                        next[0].style.webkitTransform = "translateX(0px)";
                        current[0].style.webkitTransform = "translateX(-" + window.innerHeight + "px)";
                    }, 100);
