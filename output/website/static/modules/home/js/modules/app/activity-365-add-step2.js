define('js/modules/app/activity-365-add-step2', ['require', 'exports', 'module', "jquery", "js/modules/libs/ui/popup/popup", "js/modules/libs/ui/calendar/calendar", "js/modules/libs/arttemplate/arttemplate", "js/modules/pkgs/jUploader", "js/modules/apis/api-aio"],function(require, exports, module) {
$ = require("jquery");
            ...
            if (startDateInput.length) {
                ...
                var startDateCalendar = new $.jcalendar({
                    ...
                    beforeShow: function(that){
                        ...
                        var month = that.formatDate(today).substr(0,7);
                        //console.log(month);
                        that.$calendar.append(_loading);
                        $.ajax({
                            url: CHOISE_DATE_URL,
                            type: 'POST',
                            dataType: 'json',
                        ...
                        .done(function(data) {
                            if (data.code == "0") {
                                for (var i = 0,length = data.data.length; i < length; i++) {
                                    that.$calendar.find('[data="' + data.data[i] + '"]').addClass("dc_unclick");
                                }
                            }
                            console.log("success");
                        })
    ...
    (function() {
        ...
        parent.find(".cover .js-uploader").each(function() {
            ...
            $.jUploader({
                ...
                action: complexModuleUrl, // 这里设置上传处理接口
                attrName: "pimage",
                otherData: ajaxData,
                // 上传完成事件
                onUpload:function(){
