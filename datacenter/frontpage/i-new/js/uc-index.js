$(function(){
   ...
   //      if($(e.target).closest(".reset_select").length){
   //          $(e.target).closest('.select-wrap').addClass('active');
   //      }else{
   ...
   // });
    ...
    $(".left-menu li").click(function(){
       ...
       $(".rightcontent").eq(index).show().siblings().hide();
        ...
        .done(function(data) {
            if(data.code == 0){
                ...
                for(var i = 0;i<data.data.length;i++){
                    // var city_opt=$('<option></option>').attr({value:data.data[i].id}).text(data.data[i].name).appendTo(city_box);
    ...
    // 进度条
      var markNum = $(".mark").text()-0;
      var lineWidth = (markNum/100)*288;
